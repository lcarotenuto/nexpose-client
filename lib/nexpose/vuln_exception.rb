module Nexpose
  ...
  class Connection
    ...
    def list_vuln_exceptions(status = nil)
      ...
      results = []
      req = Nexpose::AJAX.get(self, "/api/experimental/vulnerability_exceptions")
      data = JSON.parse(req, object_class: OpenStruct)

      data._resources.each do |vuln_excep|
        ve = VulnException.new(vuln_excep.id, vuln_excep.scope.type, vuln_excep.submit.reason, vuln_excep.state)
        ve.vuln_id           = vuln_excep.scope.vulnerabilityID
